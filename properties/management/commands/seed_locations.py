"""
Seed all 36 Nigerian states + FCT and their Local Government Areas (LGAs).

Run once (or safely re-run any time - every row is get_or_create'd, so this
never creates duplicates or touches existing rows):

    python manage.py seed_locations

Without this, an admin has to manually create every State and every one of
Nigeria's 774 LGAs one at a time through Django Admin - real, static,
public data that never needed manual entry in the first place.
"""
...
# Nigeria's 36 states + the Federal Capital Territory, each with their
# official LGAs. This is standard public reference data (unchanged for
# decades), but transcribing 774 names by hand carries real risk of a
# typo or two slipping through - if you spot one, it's a one-line fix in
# Django Admin (or re-run this command after correcting the entry below).
NIGERIA_STATES_LGAS = {
    "Abia": [
    ...
    "Plateau": [
        "Bokkos", "Barkin Ladi", "Bassa", "Jos East", "Jos North", "Jos South",
...
class Command(BaseCommand):
    help = "Seed all 36 Nigerian states + FCT and their LGAs. Safe to re-run."
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for state_name, lga_names in NIGERIA_STATES_LGAS.items():
                ...
                for lga_name in lga_names:
                    _, created = LGA.objects.get_or_create(
                        state=state,
                        name=lga_name,
                        defaults={'slug': slugify(lga_name)},
                    )
                    if created:
                        lgas_created += 1
        ...
        self.stdout.write(self.style.SUCCESS(
            f"Done. Created {states_created} new state(s) and {lgas_created} new LGA(s). "
            f"Database now has {total_states} states and {total_lgas} LGAs total."
        ))